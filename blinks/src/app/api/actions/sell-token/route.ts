...
import { getBuyTokenInstructionAsync, getSellTokenInstructionAsync } from "@solanacore/bonding_curve"
...
export async function GET(request: Request) {
  ...
  const payload: ActionGetResponse = {
    ...
    description: "Sell tokens from the bonding curve & get SOL",
    label: "Sell Token",
    links: {
      actions: [
        {
...
export async function POST(request: Request) {
  try {
    ...
    const maxTokenParam = maxTokenParamRaw.replace(/[^0-9.]/g, "") // remove underscores or junk
    const maxSol = Number(maxTokenParam)

    if (isNaN(maxSol) || maxToken <= 0) {
    ...
    const instruction = await getSellTokenInstructionAsync(
      {
        ...
        maxToken, // make sure it's a valid primitive number here
      },
